...
public class APIManger {
    ...
    private static final int KEY_REQUEST_DEFAULT_ITEMS = 394;
    ...
    public void downloadFile(String link, String fileName) {
        AKDownloadTask downloadTask = new AKDownloadTask(context, fileName);
    ...
    public void download(String url, final String fileName) {
        ...
        ProgressHelper.addProgressResponseListener(client, progressResponseListener).newCall(request1).enqueue(new Callback() {
            ...
            @Override
            public void onResponse(Call call, Response response) throws IOException {
                Log.e("Download", response.body().string());
                byte[] data = response.body().bytes();

                File directory = context.getExternalFilesDir("items");
                File fileWithDir = new File(directory, fileName);
                FileOutputStream out = new FileOutputStream(fileWithDir);
                out.write(data);
                out.close();
            }
        });
