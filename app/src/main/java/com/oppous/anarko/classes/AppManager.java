...
//    public List<Anarko> arrAnarko = new ArrayList<Anarko>();
    ...
    // Storage
    public File getDirectory (String folder){
        File mydir = this.context.getExternalFilesDir(folder); //this.context.getDir(AppManager.Folder, Context.MODE_PRIVATE);
        mydir.mkdirs();
        return mydir;
    }

    public FileOutputStream getFileStream(String folder, String fileName){
        File fileWithinMyDir = new File(this.getDirectory(folder), fileName);
        try{
